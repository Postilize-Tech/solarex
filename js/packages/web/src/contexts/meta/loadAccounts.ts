...
export const loadAccounts = async (connection: Connection, all: boolean) => {
  ...
  const promises = [
    ...
    ,
    getProgramAccounts(connection, SOLAREX_ID).then(
      forEach(processSolarexAccounts),
    ),
    IS_BIG_STORE
      ...
      : undefined,
    getProgramAccounts(connection, SOLAREX_ID, {
      filters: [
        {
      ...
      const result = await forEach(processSolarexAccounts)(creators);
