...
export const SOLAREX_PREFIX = 'solarex';
...
export class PrizeTrackingTicket {
  key: SolarexKey = SolarexKey.PrizeTrackingTicketV1;
  metadata: string;
  supplySnapshot: BN;
  ...
  constructor(args: {
    ...
    this.key = SolarexKey.PrizeTrackingTicketV1;
...
export class AuctionManager {
  ...
  constructor(args: {
    ...
    this.safetyDepositBoxesExpected =
      this.instance.info.key == SolarexKey.AuctionManagerV2
  ...
  isItemClaimed(winnerIndex: number, safetyDepositBoxIndex: number): boolean {
    if (this.instance.info.key == SolarexKey.AuctionManagerV1) {
...
export class AuctionManagerV2 {
  ...
  constructor(args: {
    ...
    this.key = SolarexKey.AuctionManagerV2;
...
export const decodeAuctionManager = (
  ...
  return buffer[0] == SolarexKey.AuctionManagerV1
...
export class WhitelistedCreator {
  key: SolarexKey = SolarexKey.WhitelistedCreatorV1;
...
export class Store {
  key: SolarexKey = SolarexKey.StoreV1;
  public: boolean = true;
...
export class BidRedemptionTicketV2 implements BidRedemptionTicket {
  key: SolarexKey = SolarexKey.BidRedemptionTicketV2;
  winnerIndex: BN | null;
  auctionManager: StringPublicKey;
  data: number[] = [];

  constructor(args: { key: SolarexKey; data: number[] }) {
    Object.assign(this, args);
    let offset = 2;
    if (this.data[1] == 0) {
...
export class SafetyDepositConfig {
  key: SolarexKey = SolarexKey.SafetyDepositConfigV1;
...
export async function getAuctionManagerKey(
  ...
  return (
    await findProgramAddress(
      [Buffer.from(SOLAREX_PREFIX), toPublicKey(auctionKey).toBuffer()],
      toPublicKey(PROGRAM_IDS.solarex),
    )
  )[0];
}
...
export async function getWhitelistedCreator(creator: string) {
  ...
  return (
    await findProgramAddress(
      [
        Buffer.from(SOLAREX_PREFIX),
        toPublicKey(PROGRAM_IDS.solarex).toBuffer(),
