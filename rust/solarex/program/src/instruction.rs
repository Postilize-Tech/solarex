...
#[derive(BorshSerialize, BorshDeserialize, Clone)]
pub enum SolarexInstruction {
    /// Initializes an Auction Manager V1
    ///
    ///   0. `[writable]` Uninitialized, unallocated auction manager account with pda of ['solarex', auction_key from auction referenced below]
    ///   1. `[]` Combined vault account with authority set to auction manager account (this will be checked)
    ///           Note in addition that this vault account should have authority set to this program's pda of ['solarex', auction_key]
    ///   2. `[]` Auction with auctioned item being set to the vault given and authority set to this program's pda of ['solarex', auction_key]
    ...
    ///   0. `[writable]` Uninitialized Safety deposit validation ticket, pda of seed ['solarex', program id, auction manager key, safety deposit key]
    ///   1. `[writable]` Auction manager
    ///   2. `[writable]` Metadata account
    ///   3. `[writable]` Original authority lookup - unallocated uninitialized pda account with seed ['solarex', auction key, metadata key]
    ///                   We will store original authority here to return it later.
    ///   4. `[]` A whitelisted creator entry for the store of this auction manager pda of ['solarex', store key, creator key]
    ...
    ///        Just a PDA with seed ['solarex', auction_key, bidder_metadata_key] that we will allocate to mark that you redeemed your bid
    ...
    ///   20. `[]` Safety deposit config pda of ['solarex', program id, auction manager, safety deposit]
    ...
    ///   5. `[]` Vault account
    ///   6. `[]` Safety deposit config pda of ['solarex', program id, auction manager, safety deposit]
    ...
    ///   3. `[writable]` Payout ticket info to keep track of this artist or auctioneer's payment, pda of [solarex, auction manager, winning config index OR 'participation', safety deposit key]
    ///   4. `[signer]` payer
    ///   5. `[]` The metadata
    ...
    ///   12. `[]` System program
    ///   13. `[]` Rent sysvar
    ///   14. `[]` AuctionWinnerTokenTypeTracker, pda of seed ['solarex', program id, auction manager key, 'totals']
    ///   15. `[]` Safety deposit config pda of ['solarex', program id, auction manager, safety deposit]
    EmptyPaymentAccount(EmptyPaymentAccountArgs),

    /// Given a signer wallet, create a store with pda ['solarex', wallet] (if it does not exist) and/or update it
    /// (if it already exists). Stores can be set to open (anybody can publish) or closed (publish only via whitelist).
    ///
    ///   0. `[writable]` The store key, seed of ['solarex', admin wallet]
    ...
    /// a PDA with seed ['solarex', store key, creator key] if it does not already exist to store attributes there.
    ///
    ///   0. `[writable]` The whitelisted creator pda key, seed of ['solarex', store key, creator key]
    ///   1. `[signer]`  The admin wallet
    ///   2. `[signer]`  Payer
    ///   3. `[]` The creator key
    ///   4. `[]` The store key, seed of ['solarex', admin wallet]
    ///   5. `[]` System
    ///   6. `[]` Rent sysvar
    ...
    ///       pda of ['solarex', store key, creator key] where creator key comes from creator list of metadata
    ...
    ///   15. `[]` System
    ///   16. `[]` Rent sysvar
    ///   17. `[writable]` Prize tracking ticket (pda of ['solarex', program id, auction manager key, metadata mint id])
    ...
    ///   6. `[]` Prize tracking ticket (pda of ['solarex', program id, auction manager key, metadata mint id])
    ...
    ///   12. `[]` Store
    ///   13. `[]` Rent sysvar
    ///   14. `[]` Safety deposit config pda of ['solarex', program id, auction manager, safety deposit]
    ///      This account will only get used in the event this is an AuctionManagerV2
    ...
    ///   4. `[]` Safety deposit box account
    ///   5. `[]` Vault account
    ///   6. `[writable]` Safety deposit config pda of ['solarex', program id, auction manager, safety deposit]
    ///      This account will only get used in the event this is an AuctionManagerV2
    ///   7. `[]` Auction
    ///   8. `[]` Your BidderMetadata account
    ...
    ///   18.  `[writable]` The accept payment account for the auction manager
    ///   19.  `[writable]` The token account you will potentially pay for the open edition bid with if necessary.
    ///   20. `[writable]` Prize tracking ticket (pda of ['solarex', program id, auction manager key, metadata mint id])
    ...
    ///
    ///   0. `[writable]` Uninitialized, unallocated auction manager account with pda of ['solarex', auction_key from auction referenced below]
    ///   1. `[writable]` AuctionWinnerTokenTypeTracker, pda of seed ['solarex', program id, auction manager key, 'totals']
    ///   2. `[]` Combined vault account with authority set to auction manager account (this will be checked)
    ///           Note in addition that this vault account should have authority set to this program's pda of ['solarex', auction_key]
    ///   3. `[]` Auction with auctioned item being set to the vault given and authority set to this program's pda of ['solarex', auction_key]
    ...
    /// A stateful call, this will error out if you call it a second time after validation has occurred.
    ///   0. `[writable]` Uninitialized Safety deposit config, pda of seed ['solarex', program id, auction manager key, safety deposit key]
    ///   1. `[writable]` AuctionWinnerTokenTypeTracker, pda of seed ['solarex', program id, auction manager key, 'totals']
    ///   2. `[writable]` Auction manager
    ///   3. `[writable]` Metadata account
    ///   4. `[writable]` Original authority lookup - unallocated uninitialized pda account with seed ['solarex', auction key, metadata key]
    ///                   We will store original authority here to return it later.
    ///   5. `[]` A whitelisted creator entry for the store of this auction manager pda of ['solarex', store key, creator key]
    ///   where creator key comes from creator list of metadata, any will do
...
pub fn create_deprecated_init_auction_manager_v1_instruction(
    ...
    Instruction {
        ...
        data: SolarexInstruction::DeprecatedInitAuctionManagerV1(settings)
            .try_to_vec()
            .unwrap(),
    }
}
...
pub fn create_init_auction_manager_v2_instruction(
    ...
    Instruction {
        ...
        data: SolarexInstruction::InitAuctionManagerV2(InitAuctionManagerV2Args {
            amount_type,
            length_type,
            max_ranges,
        })
...
pub fn deprecated_create_validate_participation_instruction(
    ...
    Instruction {
        ...
        data: SolarexInstruction::DeprecatedValidateParticipation
            .try_to_vec()
            .unwrap(),
    }
}
...
pub fn create_deprecated_validate_safety_deposit_box_v1_instruction(
    ...
    Instruction {
        program_id,
        accounts,
        data: SolarexInstruction::DeprecatedValidateSafetyDepositBoxV1
            .try_to_vec()
            .unwrap(),
    }
}
...
pub fn create_validate_safety_deposit_box_v2_instruction(
    ...
    Instruction {
        program_id,
        accounts,
        data: SolarexInstruction::ValidateSafetyDepositBoxV2(safety_deposit_config)
            .try_to_vec()
            .unwrap(),
    }
}
...
pub fn create_redeem_bid_instruction(
    ...
    Instruction {
        ...
        data: SolarexInstruction::RedeemBid.try_to_vec().unwrap(),
    }
}
...
pub fn create_redeem_full_rights_transfer_bid_instruction(
    ...
    Instruction {
        ...
        data: SolarexInstruction::RedeemFullRightsTransferBid
            .try_to_vec()
            .unwrap(),
    }
}
...
pub fn create_deprecated_redeem_participation_bid_instruction(
    ...
    Instruction {
        ...
        data: SolarexInstruction::DeprecatedRedeemParticipationBid
            .try_to_vec()
            .unwrap(),
    }
}
...
pub fn create_start_auction_instruction(
    ...
    Instruction {
        ...
        data: SolarexInstruction::StartAuction.try_to_vec().unwrap(),
    }
}
...
pub fn create_set_store_instruction(
    ...
    Instruction {
        program_id,
        accounts,
        data: SolarexInstruction::SetStore(SetStoreArgs { public })
            .try_to_vec()
            .unwrap(),
    }
}
...
pub fn create_deprecated_populate_participation_printing_account_instruction(
    ...
    Instruction {
        program_id,
        accounts,
        data: SolarexInstruction::DeprecatedPopulateParticipationPrintingAccount
            .try_to_vec()
            .unwrap(),
    }
}
...
pub fn create_decommission_auction_manager_instruction(
    ...
    Instruction {
        program_id,
        accounts,
        data: SolarexInstruction::DecommissionAuctionManager
            .try_to_vec()
            .unwrap(),
    }
}
...
pub fn create_redeem_printing_v2_bid_instruction(
    ...
    Instruction {
        ...
        data: SolarexInstruction::RedeemPrintingV2Bid(RedeemPrintingV2BidArgs {
            edition_offset,
            win_index,
        })
        .try_to_vec()
...
pub fn create_withdraw_master_edition(
    ...
    Instruction {
        ...
        data: SolarexInstruction::WithdrawMasterEdition
            .try_to_vec()
            .unwrap(),
    }
}
...
pub fn create_redeem_participation_bid_v3_instruction(
    ...
    Instruction {
        ...
        data: SolarexInstruction::RedeemParticipationBidV3(RedeemParticipationBidV3Args {
            win_index,
        })
        .try_to_vec()
        .unwrap(),
