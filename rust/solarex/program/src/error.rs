...
/// Errors that may be returned by the Solarex program.
#[derive(Clone, Debug, Eq, Error, FromPrimitive, PartialEq)]
pub enum SolarexError {
    ...
    /// Vault's authority does not match the expected pda with seed ['solarex', auction_key]
    #[error("Vault's authority does not match the expected ['solarex', auction_key]")]
    VaultAuthorityMismatch,

    /// Auction's authority does not match the expected pda with seed ['solarex', auction_key]
    #[error(
        "Auction's authority does not match the expected pda with seed ['solarex', auction_key]"
    )]
    ...
    /// The bid redemption key does not match the expected PDA with seed ['solarex', auction key, bidder metadata key]
    #[error("The bid redemption key does not match the expected PDA with seed ['solarex', auction key, bidder metadata key]")]
    ...
    /// The original authority lookup does not match the expected PDA of ['solarex', auction key, metadata key]
    #[error("The original authority lookup does not match the expected PDA of ['solarex', auction key, metadata key]")]
    ...
    /// Currently solarex auctions are limited to a single participation prize
    #[error("Currently solarex auctions are limited to a single participation prize")]
...
impl PrintProgramError for SolarexError {
    fn print<E>(&self) {
        msg!(&self.to_string());
    }
}

impl From<SolarexError> for ProgramError {
    fn from(e: SolarexError) -> Self {
        ProgramError::Custom(e as u32)
    }
}

impl<T> DecodeError<T> for SolarexError {
